...
def addGeneralOptions(parser, noCluster=False, logDir=False, keepTemp=False):
    ...
    if not noCluster:
        parser.add_option("-c", "--cluster", dest="cluster", action="store",
                          help="override the default cluster head node from the config file, or 'localhost'")
    if logDir:
        parser.add_option("-l", "--logDir", dest="logDir", action="store", help="log messages to directory")
    if keepTemp:
        parser.add_option("", "--keepTemp", dest="keepTemp", action="store_true",
...
def toAsciiEscape(fileData, mimeType=None, maxBinFileSize=pubConf.maxBinFileSize, maxTxtFileSize=pubConf.maxBinFileSize,
                  minTxtFileSize=pubConf.minTxtFileSize):
